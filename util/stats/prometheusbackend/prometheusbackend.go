...
func GetNamespace(config *PrometheusConfig) string {
	return config.App + "_" + config.Role
}

// Init initializes the Prometheus be with the given namespace.
func Init(config *PrometheusConfig) {
	if config == nil || !config.Enabled {
		return
	}

	if &config.RegisterAddr != nil {
		RegistConsul(config)
	}

	namespace := GetNamespace(config)
	addr := fmt.Sprintf("%s:%d", config.Host, config.ExporterPort)

	log.Info("init prometheus backend at [%s] ", addr)
	...
	go func() {
		...
		if err != nil {
			log.Error("listen and serve exporter server is failed. err[%v]", err)
			return
		}
	}()
}
...
func (be PromBackend) publishPrometheusMetric(name string, v expvar.Var) {
	...
	default:
		log.Error("prometheus: Metric type %T (seen for variable: %s) is not covered by type switch. Add it there and to all other plugins which register a NewVarHook.", st, name)
	}
}
