...
MemoryRawVector::MemoryRawVector(const std::string &name, int dimension,
                                 int max_doc_size, const string &root_path)
    : RawVector(name, dimension, max_doc_size, root_path) {
  vector_mem_ = nullptr;
}
...
int MemoryRawVector::AddToStore(float *v, int len) {
  if (ntotal_ >= max_vector_size_ || len != dimension_) {
    return -1;
  }
  memcpy((void *)(vector_mem_ + (uint64_t)ntotal_ * dimension_), (void *)v,
         dimension_ * sizeof(float));
  return 0;
}

const float *MemoryRawVector::GetVectorHeader(int start, int end) {
  return vector_mem_ + (uint64_t)start * dimension_;
}
