...
constexpr int NUM_THREADS = 15; // the number of retrieval threads
...
float fvec_inner_product_by_idx(const float *x,                   // x
                                const float *y,                   // y
                                const std::vector<uint16_t> &ids) // indice
{
...
void _knn_cosine(
    const float *x,                    // nb of query vectors
    const float *y,                    // database vectors
    size_t d,                          // dimension
    size_t nx,                         // nb of query vectors
    size_t ny,                         // nb of database vectors
    float_minheap_array_t *res,        // output heap
    int offset,                        // docid start offset
    keep_func_t keep_func,             // filter
    const std::vector<uint16_t> *idxs, // non-zero value indice in x
    int *ntotal)                       // total
{
  size_t k = res->k;
...
#pragma omp parallel for
  for (size_t i = 0; i < nx; i++) {
    ...
    idx_t *__restrict idxi = res->get_ids(i);
    auto &idxs_ = idxs[i];
...
void __Search(idx_t nx,              // nb of query vectors
              const float *x,        // query vectors, size nx * d
              idx_t k,               // topk
              float *distances,      // output distances, size nx * k
              idx_t *labels,         // output labels, size nx * k
              size_t d,              // dimension
              const float *y,        // database vectors
              size_t ny,             // nb of database vectors
              int offset,            // docid start offset
...
void _Search(idx_t nx,              // nb of query vectors
             const float *x,        // query vectors, size nx * d
             idx_t k,               // topk
             float *distances,      // output distances, size nx * k
             idx_t *labels,         // output labels, size nx * k
             ...
             int *ntotal)           // total
{
  // used to calculate the intersection of non-zero values in query vector and
  // index vector
...
public:
  ...
  int Dump(const std::string &dir) override { return -1; }
  int Load(const std::vector<std::string> &index_dirs) { return -1; }
...
int PacinsIndex::Search(const VectorQuery *query,
  ...
  auto keep_func = [this, condition](int doc) {
    ...
    return (not is_del) &&
           (condition->range_query_result ? condition->range_query_result->Has(doc)
                                       : true);
  };
