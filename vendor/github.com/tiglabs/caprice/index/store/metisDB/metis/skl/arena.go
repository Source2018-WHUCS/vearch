...
var arenaPool = &Pool{New: func(size int) *Arena {
	return &Arena{
		n:   1,
		buf: make([]byte, size),
	}
...
func (p *Pool) Get(size int) *Arena {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.size == 0 {
		p.size = size
	...
	if p.size != size {
		panic("conflict arena size for pool")
	}
	if len(p.pool) > 0 {
		a := p.pool[0]
		p.pool = p.pool[1:]
...
func (p *Pool) Put(a *Arena) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.pool = append(p.pool, a)
}

func getArena(n int64) *Arena {
	a := arenaPool.Get(int(n))
	return a
}
