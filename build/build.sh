...
DISKANN_BOOST_ROOT=$ROOT/.deps/boost-${DISKANN_BOOST_VERSION}
DISKANN_BOOST_NO_SYSTEM_PATHS=ON
...
function build_engine() {
  ...
  OMP_LIB_PATH="${oneapi_root}/compiler/2025.1/lib"
  ...
  if [ ! -f "${DISKANN_BOOST_ROOT}/include/boost/dynamic_bitset.hpp" ] || [ ! -f "${DISKANN_BOOST_ROOT}/lib/libboost_program_options.so" -a ! -f "${DISKANN_BOOST_ROOT}/lib/libboost_program_options.a" ]; then
    echo "prepare fixed boost for diskann in ${DISKANN_BOOST_ROOT}"
    bash "${ROOT}/build/prepare_diskann_boost.sh" -v "${DISKANN_BOOST_VERSION}" -p "${DISKANN_BOOST_ROOT}" -j "${COMPILE_THREAD_NUM#-j}"
  fi
  rm -rf ${GAMMAOUT} && mkdir -p $GAMMAOUT
  pushd $GAMMAOUT
  cmake -DPERFORMANCE_TESTING=ON -DCMAKE_BUILD_TYPE=$BUILD_GAMMA_TYPE -DBUILD_TEST=$BUILD_GAMMA_TEST -DBUILD_THREAD_NUM=$COMPILE_THREAD_NUM -DBUILD_GAMMA_OPT_LEVEL=$BUILD_GAMMA_OPT_LEVEL -DBUILD_WITH_DISKANN=ON -DDISKANN_ROOT=$DISKANN_ROOT -DDISKANN_INCLUDE_DIR=$DISKANN_INCLUDE_DIR -DDISKANN_LIBRARY=$DISKANN_LIBRARY -DDISKANN_BOOST_ROOT=$DISKANN_BOOST_ROOT -DDISKANN_BOOST_NO_SYSTEM_PATHS=$DISKANN_BOOST_NO_SYSTEM_PATHS -DCMAKE_BUILD_RPATH="\$ORIGIN;${MKLROOT}/lib/intel64;${OMP_LIB_PATH}" -DCMAKE_INSTALL_RPATH="\$ORIGIN" -DCMAKE_INSTALL_RPATH_USE_LINK_PATH=OFF $ROOT/internal/engine/
